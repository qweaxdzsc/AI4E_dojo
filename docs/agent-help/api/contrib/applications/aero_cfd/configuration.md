...
<a id="symbol-ai4e-contrib-application-aero-cfd-configuration-convert-configuration"></a>
## `ai4e_contrib.application.aero_cfd.configuration.convert_configuration`

- **层级**：`contrib.application`
- **稳定性**：`extension`
- **定义**：`convert_configuration(config: dict) -> dict`
- **规范定义名**：`ai4e_contrib.application.aero_cfd.configuration.convert_configuration`
...
```text
convert_configuration(config: dict) -> dict
```
...
print(signature(convert_configuration))
```
...
- 模块：`ai4e_contrib.application.aero_cfd.configuration`
- 仓库相对路径：`packages/ai4e-contrib/application/aero_cfd/configuration.py:165`
- 安装源码：先调用 `ai4e_task.source_location('ai4e_contrib')`，再按模块相对路径定位。
