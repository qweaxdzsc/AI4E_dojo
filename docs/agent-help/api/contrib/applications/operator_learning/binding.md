<!-- dojo-help: {"domain": "ai4e_contrib.application.operator_learning.binding", "kind": "api", "layer": "contrib.application", "summary": "ai4e_contrib.application.operator_learning.binding 的完整源码参考与公开符号索引。", "title": "ai4e_contrib.application.operator_learning.binding", "topic_id": "module:ai4e_contrib.application.operator_learning.binding"} -->
# `ai4e_contrib.application.operator_learning.binding` API 参考

> 本页由 AST 生成，签名和源码位置来自当前工作树。用途语义优先使用源码 Docstring，完整研究调用请继续阅读关联工作流和案例。

<a id="symbol-ai4e-contrib-application-operator-learning-binding-operatorfield"></a>
## `ai4e_contrib.application.operator_learning.binding.OperatorField`

- **层级**：`contrib.application`
- **稳定性**：`extension`
- **定义**：`OperatorField(network, model)`
- **规范定义名**：`ai4e_contrib.application.operator_learning.binding.OperatorField`
...
```python
from ai4e_contrib.application.operator_learning.binding import OperatorField
```

```text
OperatorField(network, model)
```
...
源码返回标注：`OperatorField`。普通 Python 返回值由调用方直接交接；跨阶段文件必须通过案例的 `inputs.<stage>.<name>` 或登记产物交接。
...
- 模块：`ai4e_contrib.application.operator_learning.binding`
- 仓库相对路径：`packages/ai4e-contrib/application/operator_learning/binding.py:42`
- 安装源码：先调用 `ai4e_task.source_location('ai4e_contrib')`，再按模块相对路径定位。

### 相关 API

通过 `search_help('ai4e_contrib.application.operator_learning.binding')` 查询同模块符号，通过案例 ID 查询完整阶段连接。

### 不适用场景与证据边界

API 可导入或最小检查通过只证明符号存在，不证明组件兼容、训练有效、恢复一致或达到论文精度。`internal-visible` 符号不构成兼容承诺。

<a id="symbol-ai4e-contrib-application-operator-learning-binding-operatorfield-forward"></a>
## `ai4e_contrib.application.operator_learning.binding.OperatorField.forward`

- **层级**：`contrib.application`
- **稳定性**：`extension`
- **定义**：`forward(self, input, valid=None, coordinates=None)`
- **规范定义名**：`ai4e_contrib.application.operator_learning.binding.OperatorField.forward`
...
print(signature(OperatorField.forward))
```
...
- 模块：`ai4e_contrib.application.operator_learning.binding`
- 仓库相对路径：`packages/ai4e-contrib/application/operator_learning/binding.py:53`
- 安装源码：先调用 `ai4e_task.source_location('ai4e_contrib')`，再按模块相对路径定位。
...
<a id="symbol-ai4e-contrib-application-operator-learning-binding-batch"></a>
## `ai4e_contrib.application.operator_learning.binding.batch`

- **层级**：`contrib.application`
- **稳定性**：`extension`
- **定义**：`batch(arrays, ids, *, device, case, sample_points=None, point_sequence=False)`
- **规范定义名**：`ai4e_contrib.application.operator_learning.binding.batch`
...
```python
from ai4e_contrib.application.operator_learning.binding import batch
```

```text
batch(arrays, ids, *, device, case, sample_points=None, point_sequence=False)
```
...
- 模块：`ai4e_contrib.application.operator_learning.binding`
- 仓库相对路径：`packages/ai4e-contrib/application/operator_learning/binding.py:12`
- 安装源码：先调用 `ai4e_task.source_location('ai4e_contrib')`，再按模块相对路径定位。
...
<a id="symbol-ai4e-contrib-application-operator-learning-binding-build-network"></a>
## `ai4e_contrib.application.operator_learning.binding.build_network`

- **层级**：`contrib.application`
- **稳定性**：`extension`
- **定义**：`build_network(model)`
- **规范定义名**：`ai4e_contrib.application.operator_learning.binding.build_network`
...
```python
from ai4e_contrib.application.operator_learning.binding import build_network
```
...
- 模块：`ai4e_contrib.application.operator_learning.binding`
- 仓库相对路径：`packages/ai4e-contrib/application/operator_learning/binding.py:19`
- 安装源码：先调用 `ai4e_task.source_location('ai4e_contrib')`，再按模块相对路径定位。
...
<a id="symbol-ai4e-contrib-application-operator-learning-binding-construct"></a>
## `ai4e_contrib.application.operator_learning.binding.construct`

- **层级**：`contrib.application`
- **稳定性**：`extension`
- **定义**：`construct(cfg)`
- **规范定义名**：`ai4e_contrib.application.operator_learning.binding.construct`
...
- 模块：`ai4e_contrib.application.operator_learning.binding`
- 仓库相对路径：`packages/ai4e-contrib/application/operator_learning/binding.py:72`
- 安装源码：先调用 `ai4e_task.source_location('ai4e_contrib')`，再按模块相对路径定位。
