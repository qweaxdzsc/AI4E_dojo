<!-- dojo-help: {"domain": "ai4e_contrib.application.classic_networks.prediction", "kind": "api", "layer": "contrib.application", "summary": "ai4e_contrib.application.classic_networks.prediction 的完整源码参考与公开符号索引。", "title": "ai4e_contrib.application.classic_networks.prediction", "topic_id": "module:ai4e_contrib.application.classic_networks.prediction"} -->
# `ai4e_contrib.application.classic_networks.prediction` API 参考

> 本页由 AST 生成，签名和源码位置来自当前工作树。用途语义优先使用源码 Docstring，完整研究调用请继续阅读关联工作流和案例。

<a id="symbol-ai4e-contrib-application-classic-networks-prediction-evaluate"></a>
## `ai4e_contrib.application.classic_networks.prediction.evaluate`

- **层级**：`contrib.application`
- **稳定性**：`extension`
- **定义**：`evaluate(results, output, *, consume=None)`
- **规范定义名**：`ai4e_contrib.application.classic_networks.prediction.evaluate`
...
```text
evaluate(results, output, *, consume=None)
```
...
- 模块：`ai4e_contrib.application.classic_networks.prediction`
- 仓库相对路径：`packages/ai4e-contrib/application/classic_networks/prediction.py:67`
- 安装源码：先调用 `ai4e_task.source_location('ai4e_contrib')`，再按模块相对路径定位。

### 相关 API

通过 `search_help('ai4e_contrib.application.classic_networks.prediction')` 查询同模块符号，通过案例 ID 查询完整阶段连接。

### 不适用场景与证据边界

API 可导入或最小检查通过只证明符号存在，不证明组件兼容、训练有效、恢复一致或达到论文精度。`internal-visible` 符号不构成兼容承诺。

<a id="symbol-ai4e-contrib-application-classic-networks-prediction-predict-fields"></a>
## `ai4e_contrib.application.classic_networks.prediction.predict_fields`

- **层级**：`contrib.application`
- **稳定性**：`extension`
- **定义**：`predict_fields(model, arrays, record, get_batch, output, *, derived=None, originals=None)`
- **规范定义名**：`ai4e_contrib.application.classic_networks.prediction.predict_fields`
...
```text
predict_fields(model, arrays, record, get_batch, output, *, derived=None, originals=None)
```
...
- 模块：`ai4e_contrib.application.classic_networks.prediction`
- 仓库相对路径：`packages/ai4e-contrib/application/classic_networks/prediction.py:12`
- 安装源码：先调用 `ai4e_task.source_location('ai4e_contrib')`，再按模块相对路径定位。
