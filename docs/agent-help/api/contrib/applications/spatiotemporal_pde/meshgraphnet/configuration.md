<!-- dojo-help: {"domain": "ai4e_contrib.application.spatiotemporal_pde.meshgraphnet.configuration", "kind": "api", "layer": "contrib.application", "summary": "ai4e_contrib.application.spatiotemporal_pde.meshgraphnet.configuration 的完整源码参考与公开符号索引。", "title": "ai4e_contrib.application.spatiotemporal_pde.meshgraphnet.configuration", "topic_id": "module:ai4e_contrib.application.spatiotemporal_pde.meshgraphnet.configuration"} -->
# `ai4e_contrib.application.spatiotemporal_pde.meshgraphnet.configuration` API 参考

> 本页由 AST 生成，签名和源码位置来自当前工作树。用途语义优先使用源码 Docstring，完整研究调用请继续阅读关联工作流和案例。

<a id="symbol-ai4e-contrib-application-spatiotemporal-pde-meshgraphnet-configuration-resolve"></a>
## `ai4e_contrib.application.spatiotemporal_pde.meshgraphnet.configuration.resolve`

- **层级**：`contrib.application`
- **稳定性**：`extension`
- **定义**：`resolve(config: dict) -> dict`
- **规范定义名**：`ai4e_contrib.application.spatiotemporal_pde.meshgraphnet.configuration.resolve`
...
- 模块：`ai4e_contrib.application.spatiotemporal_pde.meshgraphnet.configuration`
- 仓库相对路径：`packages/ai4e-contrib/application/spatiotemporal_pde/meshgraphnet/configuration.py:6`
- 安装源码：先调用 `ai4e_task.source_location('ai4e_contrib')`，再按模块相对路径定位。

### 相关 API

通过 `search_help('ai4e_contrib.application.spatiotemporal_pde.meshgraphnet.configuration')` 查询同模块符号，通过案例 ID 查询完整阶段连接。
