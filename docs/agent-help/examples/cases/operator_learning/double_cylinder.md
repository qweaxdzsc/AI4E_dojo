<!-- dojo-help: {"case_ids": ["operator_learning.double_cylinder"], "domain": "operator_learning", "kind": "case", "layer": "example", "summary": "double_cylinder的算子学习研究", "tasks": ["regular_grid", "scalar_field", "model", "derived", "consume"], "title": "operator_learning.double_cylinder", "topic_id": "case:operator_learning.double_cylinder"} -->
# `operator_learning.double_cylinder`

- 类型：`standalone`
- 用途：double_cylinder的算子学习研究
- 资源路径：`examples/operator_learning/double_cylinder`

double_cylinder的算子学习研究
...
- 数据集：`double_cylinder`
- 入口：`pipeline.py`
- Recipe 来源：`operator_learning/double_cylinder`
...
来源：案例 README；SHA256 `0c05114f79e8545d884e365b47d07883893155defec4593446854d2fbef9f316`。

### double_cylinder 算子网络研究

完整流程为rawprep → trainprep → train → infer → post，Python正文显示每步交接。使用 `copy_example("operator_learning.double_cylinder", "./study")` 复制后，设置config.yaml的inputs.rawprep.source、run_root和data_root；有共享准备时直接绑定inputs.train.preparation与inputs.infer.preparation，选择train/infer/post。

复制后可改写模型 binding、训练参数或阶段正文，三帧历史与下一帧目标的时间语义须保持显式。
