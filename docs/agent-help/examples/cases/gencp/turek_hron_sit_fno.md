<!-- dojo-help: {"case_ids": ["gencp.turek_hron_sit_fno"], "domain": "gencp", "kind": "case", "layer": "example", "summary": "耦合物理场；独立场训练后以明确顺序组合生成。", "tasks": ["field_sequence", "coupled_fields", "model", "objective", "neutron_condition", "solid_condition", "fluid_condition"], "title": "gencp.turek_hron_sit_fno", "topic_id": "case:gencp.turek_hron_sit_fno"} -->
# `gencp.turek_hron_sit_fno`

- 类型：`standalone`
- 用途：耦合物理场生成研究
- 资源路径：`examples/gencp/turek_hron_sit_fno`
...
来源：案例 README；SHA256 `245df34f99db45da4a17374042c951e4936f3eb98bb19df18a394dce835b81a3`。
...
- 从目录外运行时使用 `python /path/to/turek_hron_sit_fno/pipeline.py`，单阶段可直接运行同目录阶段脚本；配置中的 `inputs.*`、`run_root` 和 `data_root` 是唯一交接路径。
...
- 模型：`GenCP`；数据集：`turek_hron_sit`；依赖：`ai4e-core`、`ai4e-contrib`，以及配置中声明的可选模型依赖。
...
本目录是 `完整案例`。Agent 先读取帮助主题 `case:gencp.turek_hron_sit_fno`，再按主题关联的 workflow 和 API 参考核对输入、函数签名、产物与证据边界。支持 Python 帮助 API 时可调用：

```python
import ai4e_task as task
print(task.read_help_topic("case:gencp.turek_hron_sit_fno")["content"])
```
