<!-- dojo-help: {"case_ids": ["classic_networks.darcy"], "domain": "classic_networks", "kind": "case", "layer": "example", "summary": "二维系数到解场；保留原始物理输入、训练分片统计和固定结果消费。", "tasks": ["regular_grid", "scalar_field", "model", "derived", "consume"], "title": "classic_networks.darcy", "topic_id": "case:classic_networks.darcy"} -->
# `classic_networks.darcy`

- 类型：`standalone`
- 用途：二维系数到解场的经典基础结构研究
- 资源路径：`examples/classic_networks/darcy`

二维系数到解场；保留原始物理输入、训练分片统计和固定结果消费。
...
- 数据集：`darcy`
- 入口：`pipeline.py`
- Recipe 来源：`classic_networks/darcy`
...
来源：案例 README；SHA256 `11ae9449a106b07dde2aa81bc0a7d6700122b0261d7ed055bb3c03658243c1e1`。
