<!-- dojo-help: {"case_ids": ["safediffcon.tokamak"], "domain": "safediffcon", "kind": "case", "layer": "example", "summary": "控制轨迹；含预训练、安全校准及控制响应，区别于普通预测。", "tasks": ["field_sequence", "control_trajectory", "application", "reader", "transform", "model", "objective", "guide", "metrics", "derived"], "title": "safediffcon.tokamak", "topic_id": "case:safediffcon.tokamak"} -->
# `safediffcon.tokamak`

- 类型：`standalone`
- 用途：控制扩散研究
- 资源路径：`examples/safediffcon/tokamak`

控制轨迹；含预训练、安全校准及控制响应，区别于普通预测。

- 数据形态：field_sequence, control_trajectory
- 训练机制：iteration, diffusion_control
- 替换入口：application, reader, transform, model, objective, guide, metrics, derived
...
来源：案例 README；SHA256 `68434d88dece2b9518db43f2c37d2236b712b3a613ecb05d9e6d660c525cfe04`。

### SafeDiffCon · tokamak

本目录可整体复制，直接运行pipeline.py或作为Task新建来源；不依赖仓库外的recipe脚本。六阶段正文允许自由编辑。

配置及命令见研究模板说明（原参考 `../../../维护源/README.md` 未随此帮助交付；实际阶段源码请在复制案例内阅读），实际安装与训练证据见验收记录（原参考 `../../../运行记录/safediffcon-acceptance.md` 未随此帮助交付；实际阶段源码请在复制案例内阅读）。config.yaml和quick.yaml保留同一缩小科研默认；每案例累计180分钟仍由原账本监督。
...
- 从目录外运行时使用 `python /path/to/tokamak/pipeline.py`，单阶段可直接运行同目录阶段脚本；配置中的 `inputs.*`、`run_root` 和 `data_root` 是唯一交接路径。
...
- 模型：`SafeDiffCon`；数据集：`tokamak`；依赖：`ai4e-core`、`ai4e-contrib`，以及配置中声明的可选模型依赖。
- 阶段顺序：`rawprep → trainprep → train → posttrain → infer → post`。单阶段入口是同名 `.py` 文件；参数覆盖使用 `--set`，不通过隐藏任务文件传参。
...
本目录是 `完整案例`。Agent 先读取帮助主题 `case:safediffcon.tokamak`，再按主题关联的 workflow 和 API 参考核对输入、函数签名、产物与证据边界。支持 Python 帮助 API 时可调用：

```python
import ai4e_task as task
print(task.read_help_topic("case:safediffcon.tokamak")["content"])
```
