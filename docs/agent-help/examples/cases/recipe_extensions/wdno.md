<!-- dojo-help: {"case_ids": ["recipe_extensions.wdno"], "domain": "recipe_extensions", "kind": "case", "layer": "example", "summary": "小波预测网络、目标、训练局部策略与能量审计；先物化完整基案例，再按扩展说明修改。", "tasks": ["regular_grid", "field_sequence", "network", "objective", "optimizer", "update_step", "post"], "title": "recipe_extensions.wdno", "topic_id": "case:recipe_extensions.wdno"} -->
# `recipe_extensions.wdno`

- 类型：`extension`
- 用途：参考变体与 Agent 组件组装示例
- 资源路径：`examples/recipe_extensions/wdno`

小波预测网络、目标、训练局部策略与能量审计；先物化完整基案例，再按扩展说明修改。

- 数据形态：regular_grid, field_sequence
- 训练机制：iteration, diffusion_prediction
- 替换入口：network, objective, optimizer, update_step, post
- 限制：覆盖文件集不能直接当完整案例运行；未声明的行为沿用基案例。
- 限制：仅声明扩展演示范围，不据此扩大数值精度验收。
- 基案例：`wdno.burgers_base`
- 覆盖文件：
  - `variants.py`
  - `audit.py`
...
来源：案例 README；SHA256 `74ca7f332c1564b0b99dc86f6f98468c5b0e605a8c6a16ddc0e58669ed7b05f6`。
...
直接脚本与Task执行同一份pipeline；Task复制研究目录后，后续修改应在任务副本进行。新建项目/任务后分别提交准备、训练、推理和post即可；公开使用说明见主模板（原参考 `../../../维护源/README.md` 未随此帮助交付；实际阶段源码请在复制案例内阅读），实际验证工具见WDNO验证入口（原参考 `../../../运行验证说明` 未随此帮助交付；实际阶段源码请在复制案例内阅读）。
...
- 基案例：`wdno.burgers_base`。先 `copy_example` 物化；返回的 `documentation.entry` 可定位基案例和扩展的说明副本。
- 本变体可改：`network`、`objective`、`optimizer`、`update_step`、`post`。
- 覆盖/新增文件：`variants.py`、`audit.py`、`configure_variant.py`、`config.yaml`、`variant_training.py`、`pipeline.py`。逐项读这些文件的输入输出和基案例消费者，其他阶段继续继承。
...
基案例完整说明：[本地正文](../wdno/burgers_base.md)。
