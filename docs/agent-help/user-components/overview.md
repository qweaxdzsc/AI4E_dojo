...
用户组件可以拥有自己的输入输出类型，不要求继承 Dojo 基类，也不要求插件注册服务。完整新训练任务复制最接近的 standalone，在阶段脚本中明确连接与转换，将算法和业务适配放入本地组件；没有完全匹配的模型案例时也可以替换其组件。已有成熟工程的局部修改可在原调用处接入。可持久化配置保存全限定 import 路径和构造参数；闭包和 lambda 不能作为可重建声明。
...
2. 完整新任务选择最接近的 standalone 并复制，检查每个阶段的输入输出与目标协议差异；单工具或成熟工程局部接入直接参考对应能力教程。
3. 保留流程编排，替换不相容的科学设置与组件，运行目标基线并保存配置、阶段摘要和固定结果；不要求先训练无关领域模型。
...
有状态组件的可选保存、只读预检与恢复见[用户内存状态](checkpoint-state.md)；无需统一基类，无状态组件不增加义务。
