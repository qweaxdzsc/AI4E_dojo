...
### 5.2 建模能力的组合尺度：block、网络阶段与完整架构

本节为稳定设计边界。经典网络的block、网络阶段及完整模型已按此实现，实际组合、来源变体与训练/安装边界见[专项验收](../.context/mvp/classic-networks-acceptance.md)；不因目录齐备推断任意重组有效或论文精度达标。本批分工及范围见[主计划](../.cursor/plans/classic-networks-main.plan.md)。

网络计算按三个可直接调用的组合尺度组织，均属于 `core/abilities/modeling`，不增加包依赖层或全仓统一组件协议：

- `modules/` 保存具有完整计算意义的 block，例如多层特征变换、卷积特征提取、残差分支、注意力交互、消息传播、循环特征、分辨率转换及跳连融合。粒度通常大于单个框架算子；不为 Linear、激活、一次加法或 reshape 机械增加包装。已有独立数学原语可继续存在，不因这条网络拆分原则被强制合并。
- `stages/` 保存能独立取用的网络子结构，例如同尺度卷积/残差段、多尺度编码器与解码器、注意力堆叠、图处理段、多层循环段。这里的网络阶段与 `run.stage` 或 rawprep/train/infer 等业务阶段无关。没有独立复用价值时不强造中间层，例如简单 MLP 可直接使用前馈 block。
- `models/` 保存由这些公共 block 与网络阶段实际组成的完整中立架构，提供直接使用的标准组合。完整架构本身也是 ability；特定模型的不可中立化结构仍按5.1归属判定处理。

可中立计算的依赖方向为完整架构 → 网络阶段 → block → 框架算子；允许完整架构直接使用 block。block 不反向依赖完整模型，不持有数据集、训练器或运行会话。前馈能力可以同时服务 MLP、注意力前馈、图编码/更新和预测头；完整模型不得再维护一套与公开 block 相同的私有计算。代码来源与许可仍逐项保留，抽取不能消除原许可义务。

**抽取以可替换的计算职责为边界。** 形状相同不表示可替换；归一化前后位置、残差分支与投影、激活、消息方向、聚合和循环状态顺序属于算法定义。标准注意力与几何/物理注意力分别声明语义，不用一个巨型 block 的分支开关统一所有模型。单步循环可复用已有框架单元；状态显式交接，不藏在跨样本可变对象中。

**拆装由公开构造位置承载。** 编码器、瓶颈、解码器、消息处理段和读出等实际边界接受普通函数或网络对象；子模块采用框架正常注册机制，使参数、设备、模式和检查点可追踪。用户可以独立调用公开阶段、消费多尺度特征或替换其中一段，不必复制整模型或访问私有属性。架构变体使用独立结构声明和检查点；不承诺任意替换后旧权重仍兼容。

**跨表示显式转换。** 网格声明空间轴和分辨率，token声明顺序、位置与mask，图声明节点/边/邻接和实体身份，序列声明时间轴、长度与状态。patch编码和重建记录必要空间信息；图与token连接保留节点身份；不能用元素数相同的reshape冒充语义兼容。可训练投影与网络布局属于建模计算，物理来源取样/插值属于数据或变换能力，字段、单位和监督目标由局部业务连接解释。

**组合能力须有实际证据。** 按任务范围分别核对公开block对照、经典架构重建和跨架构替换/重组；检查前向、梯度、注册参数、状态及保存读回，不把目录或组件类存在当成可组合性。重组范围按实际风险选定，不把固定混合模型或数据集写成全仓强制清单。已有公开接口和历史权重保留；调整旧内部实现时使用原基线验等，不改摘要来通过。

**算子与传统代理沿用同一组合尺度。** DeepONet由公开前馈和分支—主干读出组成，查询布局和多输出潜空间明确；FNO由谱卷积、Fourier块/阶段、公开升降通道组成，规则格空间域不能按普通样本批次切断。POD用冻结降阶基表示场，完整预测由系数代理与解码组合；RSM、RBF、Kriging消费公开多项式、径向或协方差计算，不为简单代数表达强造中间阶段。LightGBM只封装可选官方引擎的预测和完整原生状态，不宣称将其内部树算法提炼为本仓库block。

**学习算法与预测计算分开。** SVD、最小二乘、核方程、概率超参数优化和树训练进入training；冻结基投影、响应预测及条件均值/方差进入modeling。普通对象的样本分批属于inference，不调用神经网络eval或隐式改变精度。data/save保存数组、原生文本及内容摘要；应用核对字段、统计、基底和来源身份。代数模型状态读回不伪称优化器恢复；树模型追加轮次、神经检查点完整接续各按自身实际语义验证。可选依赖延迟加载，基本建模不因缺LightGBM失效。

**物理约束独立于模型。** 复用constraint现有损失，领域连接解释方程、单位、mask和可微反归一化；不增设PINN整网。可微POD解码保持系数计算图，但不将离线SVD拟合声称为端到端微分。普通非梯度拟合不会因输出可评价物理量而自动成为受约束训练。本批实现及真实验收进度见[算子与代理验收](../.context/mvp/operator-surrogates-acceptance.md)。
...
外流 recipe 显式连接同一准备、训练和推理科学链；可选 topology 只派生图缓存。AB-UPT 与公共模板保留锚点链，Transolver、MeshGraphNet 与 GeoTransolver 使用物理链，Task 不根据模型选择步骤。
...
Task 从固定 pipeline.py/config.yaml 发现普通研究入口，从 inputs.<stage>.<name> 捕获所选阶段的外部路径，并分配 run_root/data_root。不存在第二份逐案例 task-entry。可选 components.application 提供检查、评价、导出连接；缺操作只使该项不可用，Task 不猜测模型。Python 正文决定真实顺序，配置中的 stages 只选择范围。资产和具有科学口径的指标由 writer 写公共索引，管理层核验实际文件及依赖后查询、比较；发布者可声明仅使用目录内相对引用的自包含资产包，管理层整体复制并重定位管理引用，不改科学文件；未登记不等于运行失败，也不能推断研究成功。历史任务通过有原件备份、逐文件差异和回滚日志的迁移更新可编辑目录；官方目录识别已核验摘要并生成明确文件清单，Task 只执行有备份与回退的替换事务，自定义正文使用审阅后的离线候选。冻结版本与实验记录不改写。源码摘要用于来源与漂移核验，不能以模板全文不同拒绝自由连接。

应用来源固定入口、静态可发现依赖、声明的动态模块与资源，以及实际导入解析位置。来源记录 version=2；内容变化或同名遮蔽拒绝执行，新增无关能力文件允许。动态依赖通过应用模块的 SOURCE_DEPENDENCIES.modules/files 声明，资源相对声明模块定位。只覆盖应用依赖，不冻结解释器、驱动或整个第三方运行环境。历史记录缺少依赖覆盖时对应操作明确不可用，固定历史文件仍可读取，不自动补写来源。
...
应用通过可选 `inspect` 的 `describe_task` 操作声明输入类别、精确标签条件、阶段、恢复输入、共享输出和管理操作。Task 不理解这些标签的科学含义，只检查运行状态、路径与文件修订并逐项等值匹配。应用返回带作用域的不透明执行身份，Task 对同批次身份做等值检查；检查点合同、执行参数、进度含义与结果字段均由应用解释。描述按配置与源码修订缓存，新运行捕获来源；历史来源缺失或发生漂移时不得改用当前应用。

固定物理结果的既有检查适配保持自己的持久化格式，子进程接受明确提供者；它不是普通组件的通用数据协议。平台操作的 JSON 输入/输出只约束该操作。

## 11. Vis

结构图由应用准备真实输入、网络和显示参数，再调用 core 的通用跟踪能力生成固定 HTML 与来源记录。通用能力保护权重、缓冲区、模式和随机流，失败清理本次临时产物并保留上次完整结果；不读取案例配置或导入 Vis。AB-UPT 阶段包装归贡献应用。Task 检查进程仅交接可序列化记录；Server 登记结果引用，Vis/Web 消费固定文件。
