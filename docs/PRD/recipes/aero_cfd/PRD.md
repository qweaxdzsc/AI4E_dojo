...
同一个外流模板提供九个独立 example：汽车/NASA 的 AB-UPT、Transolver-3 五例，ShapeNet-Car、NASA CRM 两个静态 MeshGraphNet 例，以及两个普通 GALE GeoTransolver 例。五段配置保持不变；通过 components.model 选择模型，模型参数与准备字段相应修改。一个 example 对应一次独立训练。已有平台物理数据可从 trainprep 开始；图案例要求清单同时提供逐场 PT、VTKHDF 和实体身份。原始读取仍可独立执行。原五组交叉实验的既有预算不因新增案例改变。

九个案例使用各自明确的科学连接，保留公共阶段输入和结果交接。公共模板及 AB-UPT 明确使用锚点准备、训练和推理；三个 Transolver 与两个 MeshGraphNet 使用物理准备、物理训练及完整物理推理；两个 GeoTransolver 保留既有物理链。`trainprep.topology` 只负责图缓存及核心分区与 halo，不能据其有无决定模型流程；MeshGraphNet 缺少必需拓扑时明确拒绝。用户可在复制目录中插入或替换步骤，新增输出需要保存并交给下游读取。锚点与全点评价按各自实体集合比较。 数值回归保留冻结训练与预测正文，并在测试连接层核验准备、归一化和权重来源。恢复分别使用参考与当前流程自己的完整状态；旧参考遗漏的独立采样器状态只为本次测试旁录，绑定对应检查点摘要。此证据不表示任意旧契约检查点可直接恢复到现行科学链。
...
使用 ${data_root}/train 等明确路径表达式，各项均可独立覆盖。变量先展开，相对路径按配置文件解析；未知变量与循环引用拒绝。公开案例默认把原始输入放在复制目录的 `../inputs/<dataset>`，已处理数据从 `../datasets/<dataset>` 交接，数据产物和运行记录分别写入 `../data/<case-id>` 与 `../runs/<case-id>`，不把任何一类写进 recipe 代码目录。未配置分片及未执行归一化目录不创建。统计只拟合本次完整训练分片，失败不发布完整清单；输入快照仅保存最终生效配置。
...
准备引用可跨进程使用；修改输入声明、组件或数据内容后必须重新准备。通用准备适配器允许模型不声明 AB-UPT 专属的 `model.data_specs`，由模型自己的低层 `prepare_inputs` 解释字段绑定；Transolver 因此直接消费平面归一化点场。历史 post 的公开兼容门面可只读导入摘要、数据内容和归一化均通过校验的通用 version=2 准备，保留原 digest，不用冻结业务声明阻挡当前配置。检查模式不发布准备产物或训练检查点；整条流水线检查不会虚构未落盘的上游数据，下游检查列为待执行。
...
兼容模板保留 post 计算参数，但它们只用于历史 post-only 执行，原生 infer 不读取这些值。原生捕获配置可删除旧 post 计算键，仅保留 results；结果通过 inputs.post.results 指定。两个结果引用同时存在且不相同时拒绝，避免打开错误结果。连续流水线直接传递推理结果；新后处理不产生第二轮预测，不修改历史准备和检查点。旧脚本及旧配置仍使用原计算入口，不批量重写。
...
任务比较迁移同时更新当前 `task-entry.json` 的采样选择器到 `model/sampling`；旧运行继续按本次运行冻结的 entry 与 inputs/config.yaml 读取旧 `trainprep/sampling`。真实 task 双运行测试证明工作目录后续改动不改变既有比较语义。模板明确登记组件与应用入口；历史缺少应用来源的操作明确不可用，不由 Task 或浏览器补用默认领域提供者。
