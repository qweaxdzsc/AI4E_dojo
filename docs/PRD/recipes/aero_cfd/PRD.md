...
- 案例从数据集声明补充原始处理默认配置，显式参数覆盖默认值，配置路径相对配置文件解析。Python 保留真实步骤顺序，用户在复制案例中通过公开字段映射插入计算，并声明实体、单位与物理状态；新增输出必须加入保存与后续消费。新模板默认写出 PT，并可在 `rawprep.formats` 同时声明 Zarr；旧 `rawprep.format` 单键继续有效。数据集默认已改为 `formats: [pt]`；与平台选项合并时以页面 `formats` 覆盖，不向用户报冲突。三个 ShapeNet 官方案例显式写 `rawprep.vtkhdf: true`，案例写 false 会盖掉清单默认；NASA 等未接入集不显示也不强开。已保存任务保持原值，不因清单默认改写。新任务默认保存归一化副本；坐标场可勾统一空间（配置里仍写 `method: coordinate`），方法映射到 `[0, 1]`，场上 `scale` 默认 1。AB-UPT 官方案例坐标预填 `scale: 1000`，Transolver-3 官方案例为 1。研究者改平台数据集名称只动 `dataset.processed_name`。
