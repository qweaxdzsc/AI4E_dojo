...
同一套阶段脚本支持汽车 AB-UPT 与 NASA CRM Transolver-3 两个可复制配置案例。脚本只选择组件和调用业务装配，模型与数据集切换无需改框架源码或编写训练循环。
...
同一流水线可以从命令或普通脚本运行。托管运行保存来源与捕获代码，既有准备、训练和 post 仍由原业务装配执行。相对误差指标声明字段、域、单位、分片及采样定义，使用固定运行结果比较，不由任务重算物理误差。
