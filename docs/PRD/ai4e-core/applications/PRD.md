...
锚点推理一次预测可同时交付评价和保存，两项操作分别记录实际完成的样本数与失败状态；内层评价结束后恢复外层保存的文件归属。评价成功但保存失败时不抹去已完成评价，部分样本失败保留先前交付，完成数量不能从运行终态补造。历史进度账本不回写。
...
接收普通模型、取批、目标函数、优化器和调度器，共用完整状态恢复与执行。检查点间隔可选，默认保持500次有效更新；目标是累计更新数，取消/限时保留可恢复状态。验证和检查点可以分别指定周期；同一次更新同时触发时先验证再保存，使用户策略状态与当次评价一致。旧底层迭代调用未指定独立保存周期时继续跟随评价。自定义迭代器不支持新参数且要求不同周期时，在更新模型前拒绝，不静默忽略设置。

有状态用户组件可显式提供保存、只读预检和加载函数，固定兼容声明与可变内存状态分开。恢复先核对完整状态名称与语义，拒绝缺失或悄悄丢弃状态；加载失败尽力回滚框架管理状态并明确报告回滚失败。不支持恢复函数修改外部服务或文件等不可回滚副作用，无状态组件无需登记。
