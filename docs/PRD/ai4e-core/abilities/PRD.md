...
冻结变换支持 custom 方法的 target/parameters 工厂，重建 apply/inverse 并检查来源。采样、目标、指标、预测的算法继续由能力提供，recipe 通过各自领域步骤注入，能力不读取 recipe 私有上下文。 验收导航：`.context/mvp/recipe-explicit-acceptance.md`。
...
- 清单索引按样本名重挂分片后才能读取新身份；空分片不进入索引。新划分写入准备记录，训练读这次记录。
