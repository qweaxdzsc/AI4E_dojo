...
应用模块可用 SOURCE_DEPENDENCIES 声明无法静态发现的模块与必要资源；普通科学组件无需实现该约定。本地资源相对声明模块定位，并随案例副本搬移，外部模块依赖固定实际位置和内容。

平台检查、评价和导出从所选 application 发现可选公开操作，缺项明确不可用，不默认按外流处理。可选 describe_task 按配置修订与入口来源修订缓存，描述输入位置、资产类别和标签条件、阶段、恢复输入、共享输出及管理操作；提交时随运行捕获。普通 inputs 继续只保存路径。自由脚本缺少描述仍可直接或托管执行，其未声明输入按普通文件管理。不读取旧 `task-entry.json` 或创建快照补回缺失操作；现行执行必须明确声明 application。源码摘要保留来源，不按模板全文限制用户修改。旧配置和任务需显式离线迁移并保留原件；新执行入口不暗中改写旧脚本或挂接旧格式适配器。公共约定迁移仍在专项验收中，历史任务未因此自动切换。
...
案例列表提供用途摘要、数据形态、训练机制、可替换点和限制。研究者可按说明子串或标签交集筛选，结果保持清单顺序，不作智能匹配承诺；旧条目仍可按原用途读取，缺少标签时不推断补齐。详细输入输出、修改位置和运行方式由案例正文说明，列表帮助初选，不替代接口核对。

复制完整案例时保留原脚本和根说明字节，另生成带来源摘要的说明副本。扩展同时交付基案例与变体正文，明确覆盖文件，避免只拿到基案例说明而遗漏变更。说明保留目录冲突在复制前拒绝；源说明中的未交付本机参考转为明确提示，已交付文件的链接转换到副本位置。来源记录和复制返回值给出文档入口。

离线帮助内嵌由真实案例说明生成的正文，扩展链接到同时导出的基案例主题。摘要、正文与构建副本分别维护和生成，不建立第二套手写案例目录。历史冻结副本不批量回写。
