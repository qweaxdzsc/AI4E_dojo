...
源码替换事务只接受明确文件清单、候选来源和原件内容修订，先备份再替换，任一失败恢复。官方脚本识别与迁移清单归平台目录，任务存储不保存案例特征。

共享生产使用独立占用记录与发布收据。预留、正在生产、完整可用与失败分别保存；不得按锁年龄或进程状态未知自动释放。明确覆盖开始撤下旧可用登记并重做内容，不保留旧物理修订。完成后在事务外校验完整清单与成员摘要，再提交当前资产和固定运行收据；运行目录仍只有运行记录写入器负责。

先暂存并核验文件，再在写事务内发布引用。相同幂等请求返回原对象，不同请求复用键时报冲突。失败不发布可用半成品。

#### 3. 保存可核验快照

固定来源包含应用入口、科学依赖源码摘要及安装版本。源码身份不要求与官方模板相同，但执行已捕获的科学操作时必须与捕获身份一致；不会自动重建旧环境。
...
共享数据覆盖开始前保留本次原件和登记记录。生产失败恢复旧内容与旧来源，首次生产失败则不可用；生产期间旧准备不能把构建中的同名内容视为完整数据。回退副本属于本次事务，不是数据修订产品。未知生产进程须核对，不能仅因等待超时接管。
...
给研究者一份跨项目可按名称选用的平台数据资源。正式原始处理成功后登记名称、清单路径、内容摘要和来源声明；Task 新产物位于项目共享目录，平台按来源项目与共享资产身份汇聚。旧任务路径登记继续可读，既有通用登记接口保留原语义；不复制到 contrib 源数据集，也不另造算法清单格式。归档来源任务不删除共享数据。
...
| 冲突 | processed_dataset_name_conflict | 同名已被占用且未确认覆盖 |

### 5.5 功能点清单

1. 按名称登记已处理数据集
    1.1 名称必填且符合标识规则
    1.2 同名同摘要复用，未确认覆盖时拒绝替换
    1.3 确认覆盖后替换原登记
2. 核验可用性
    2.1 路径丢失或摘要变化标为不可用
    2.2 删除来源任务不级联删除登记
...
兼容接口的工作区根下 `datasets/<name>/dataset.json` 保存名称、清单绝对路径、内容摘要、来源任务/运行和声明指纹。试跑或不完整产物不登记。名称位置由任务共享输出声明给出，提交后消费捕获的输出计划，不从当前科学配置重新猜测。单资源登记入口遇到多个输出时明确拒绝，调用者逐项登记。生产者可显式交付类别、阶段和语义标签，登记原样保留；缺标签的历史资源可列出，但不作为标签匹配成功的候选。
...
以下复用行为仅适用于旧工作区登记接口；Task 的同名重新生产总是要求显式覆盖。再次登记同一份清单时复用已有记录。同名且声明相同的新成功运行可更新指向；同名但声明或摘要指向另一份数据且未确认覆盖时拒绝，避免其他项目悄悄换数据。处理身份及差异说明由明确 application 生成，Task 只比较完整身份的类型和值。外流应用保留原统计口径，线程数与列表顺序不改变身份。旧登记缺身份时明确缺失，不能读取当前任务配置补成相同；继续覆盖需要本次明确许可。

##### 1.3 确认覆盖后替换原登记

旧工作区登记接口中，研究者明确确认后，用这次成功运行的清单、摘要、声明和来源替换同名 `dataset.json`，并更新登记时间。旧张量仍留在原任务数据目录，不复制也不级联删除。后台补登记不得静默覆盖不同声明。
