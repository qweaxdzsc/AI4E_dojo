...
兼容接口的工作区根下 `datasets/<name>/dataset.json` 保存名称、清单绝对路径、内容摘要、来源任务/运行和声明指纹。试跑或不完整产物不登记。名称写在任务配置 `dataset.processed_name`；研究者改名称只动这一项。
...
以下复用行为仅适用于旧工作区登记接口；Task 的同名重新生产总是要求显式覆盖。再次登记同一份清单时复用已有记录。同名且声明相同的新成功运行可更新指向；同名但声明或摘要指向另一份数据且未确认覆盖时拒绝，避免其他项目悄悄换数据。声明只计来源与处理产物语义：场与分量、几何能力集合、格式、VTKHDF、统计策略；不含 `rawprep.workers`，列表顺序和展开默认值不另占名称。旧登记只存指纹时，按来源任务配置还原身份再比。差异说明写出 VTKHDF 等实际变化。
