...
项目共享查询返回当前清单、来源、生产标签与可用状态。绑定按消费步骤的类别和全部标签精确匹配，缺标签或冲突拒绝；多个位置匹配时要求明确选择。共享、派生、复制及输入捕获保留标签，不根据路径猜用途。任务按入口声明绑定清单，保存当前可编辑配置与资产关联，校验配置修订；不创建新版本、不改创建快照。派生任务默认继承共享引用，显式复制则复制完整物理数据及统计依赖。覆盖后新的准备捕获同名当前内容，已有准备和检查点保持原字节并继续遵守原冻结校验。
...
历史迁移必须传入明确 application 上下文，默认只预览其识别的正式成功来源，排除试跑和失败；同名多个候选须明确选择，不自动采用最新。科学清单解释和新副本引用转换由 application 返回，Task 执行受控文件清单及内容核验；执行时复制物理成员、网格、实体身份及统计依赖，校验复制前后内容，路径仅在新副本修正。旧平台登记名称与运行时名称不一致时，可按明确选中的名称与正式运行配对迁移，保留用户选择的实际内容；不以其他同名运行替换。相同来源再次迁移直接复用，其他同名内容必须明确覆盖。失败副本不可登记为可用。历史任务、运行、准备与检查点不迁移、不重算。
