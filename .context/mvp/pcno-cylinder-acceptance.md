...
用户要求重新检查后，以独立数组字节解码、闭式形函数梯度和三边通量积分重新扫描全部6000帧训练/验证数据。原P1梯度与独立公式最大绝对差1.14e-13，边通量散度差4.79e-14，原比值复现差低于1.4e-17；未发现矩阵方向、分量、面积或读取实现错误。

原5.57%—7.14%不是COMSOL求解器残差或质量损失率。全域边界积分的净流量不平衡/流入量在全部帧中最大6.20e-8（0.00000620%）；全域平衡不代表局部逐点连续性。严格无散的解析曲场在同一网格、同一有效域经P1重建也得到7.55%—7.90%，半边长重新采样后约减半。因此原指标包含重建导数误差，不能以1%门槛判断原数据质量。旧候选损失仍未准入，未改门槛、未启动新训练；COMSOL来源离散和相容物理损失仍需验证。

详细证据见实验根 `official-source-review/divergence-recheck.md`、同名JSON及独立脚本 `recheck_divergence.py`。相关物理测试从源码导入后3项通过；主环境旧安装缺模块的首次收集失败单独保留，不计通过。未改正式环境；无新增大数组。
