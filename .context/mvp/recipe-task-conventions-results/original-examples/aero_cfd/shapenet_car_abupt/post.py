...
def post(cfg, trained=None):
    ...
    session = TrainingRun()
    reference = cfg.post.get("results") or cfg.get("infer", {}).get("results")
    if (
        cfg.post.get("results")
        and cfg.get("infer", {}).get("results")
        and cfg.post.results != cfg.infer.results
    ):
    ...
    try:
        rows = session.execute_samples(
            source.samples,
            partial(analyze_sample, cfg=cfg, output=cfg.paths.datasets.post),
            stage="post",
        )
        ...
        report = post_ops.publish_analysis(
            ...
            source=source,
            output=cfg.paths.datasets.post,
            failures=error.summary["failures"],
        )
        session.report(report, stage="post")
        raise
    report = post_ops.publish_analysis(rows, source=source, output=cfg.paths.datasets.post)
    session.report(report, stage="post")
    return report
