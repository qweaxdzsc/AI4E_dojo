    ...
    *,
    ...
    try:
        with transaction(project) as db:
            ...
            captured_runs = all_records(db, "run") if parent and kinds else []
            parent_version = get(db, "version", parent["version_id"]) if parent else {}
        stage.mkdir()
        ...
        if configuration is not None:
            ...
            OmegaConf.save(OmegaConf.create(configuration), stage / "recipe" / "config.yaml")
            entry = read_entry(stage / "recipe")
        if not parent:
        ...
        with transaction(project) as db:
            ...
            if previous is not None:
                return previous
            if parent and get(db, "task", parent["id"]) != parent:
