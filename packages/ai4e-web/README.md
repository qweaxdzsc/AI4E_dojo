...
新建任务只选择服务实际登记的数据集与模型案例；ShapeNet 目录和 NASA 三个来源文件在原始处理页用受控浏览绑定，创建和编辑弹窗不接受数据路径。字段提取按每个输出 `.pt` 一行配置，添加在列表上方，编辑和删除用图标；对话框只勾选一个物理量或坐标，格式固定 `.pt`。阶段配置仍可选附加 VTKHDF；弹窗不保存目录或改格式。准备统计只读，模型采样与固定损失行跟随当前模型字段，Transolver 切片数量在主干参数；模型设置不提供权重加载。检查、试跑、正式运行分开；训练与后处理选择既有正式产物。监控和图表只显示真实记录，缺失耗时/峰值显存明确不可用。
