...
export async function inferenceFixture(page:Page,{completed=true,samples=2000,empty=false,longNames=false,status="succeeded",legacy=false,multiplePairs=false,divergent=false,stale=false,failSecond=false,meshUnavailable=false,omitVtkExports=false}={}) {
 const state={submitted:[] as any[],exports:[] as any[],checks:[] as any[],queries:0};
 const cps=Array.from({length:12},(_,i)=>({id:`cp${i}`,run_id:i<6?'train0':'train1',name:longNames?"很长的检查点名称_".repeat(10)+i:i===0?'BEST':`ckpt_${128000-i*1000}`,revision:`sha${i}`,epoch:i+1,updates:128000-i*1000,size:2048,status:'succeeded',created_at:`2026-09-15T08:${String(20-i).padStart(2,'0')}:00Z`,compatibility:{status:'compatible'},preparation:{digest:'prepared'},evaluation:{value:.000122+i*.00001,metric:'loss',split:'validation',direction:'min',protocol:'same'}}));
 ...
 const statistics=cps.slice(0,5).map((c,i)=>({checkpoint_id:c.id,checkpoint:c.name,split:'__all__',field_id:fields[0].id,field:'Pressure',metric:'relative_l2',unit:'1',mean:(3.21+i)*.0001,median:(2.12+i)*.0001,p90:(6.12+i)*.0001,max:.00123+i*.0001,valid:samples,expected:samples,undefined:0,complete:true,prediction_seconds:18.4+i/10,throughput:100}));
 const records=cps.slice(0,5).flatMap(c=>['train','validation'].flatMap(split=>['same_sample','second_sample'].map((sample,i)=>({checkpoint_id:c.id,checkpoint:c.name,field_id:fields[0].id,label:'Pressure',split,sample,status:'succeeded',unit:'1',values:{relative_l2:(i+1)*.1},undefined:{}}))));
 if(multiplePairs){
  statistics.push(...statistics.map(r=>({...r,metric:'mae',unit:'Pa'})),...statistics.filter(r=>r.metric==='relative_l2').map(r=>({...r,field_id:fields[1].id,field:'Velocity-U',metric:'rmse',unit:'m/s'})));
  records.forEach(r=>Object.assign(r.values,{mae:.2}));records.push(...records.map(r=>({...r,field_id:fields[1].id,label:'Velocity-U',values:{rmse:.3} as any})));
 }
 let batch:any=completed?{id:'batch',name:'已完成的验证批次',status,total:10,completed:status==='succeeded'?10:3,children:[]}:undefined;
 await page.addInitScript(({completed,stale})=>{if(stale)sessionStorage.setItem('dojo.infer.p.t','gone');else if(completed)sessionStorage.setItem('dojo.infer.p.t','batch');},{completed,stale});
 ...
 else if(path.endsWith('/inference/samples')){const id=new URL(req.url()).searchParams.get('checkpoint_id');if(failSecond&&id&&id!==cps[0].id){await route.fulfill({status:400,json:{detail:'inference_preparation_missing: 检查点关联的准备或样本文件不存在，请确认数据准备仍可用'}});return;}const vtk_exports=omitVtkExports?undefined:{pointcloud:{available:true,include_truth:true},mesh:{available:!meshUnavailable,include_truth:true,structured:false,reason:meshUnavailable?'训练集没有可还原的 VTK 网格':null}};const base=divergent&&id&&id!==cps[0].id?{partitions:{test:['other_sample']},fields,metrics,selection_supported:!legacy,preparation:{digest:'other'}}:{partitions,fields,metrics,selection_supported:!legacy,preparation:{digest:'prepared'}};json=vtk_exports?{...base,vtk_exports}:base;}
 else if(/\/batches\/gone$/.test(path)){await route.fulfill({status:404,json:{detail:'文件不存在或数据根未配置'}});return;}
 ...
 else if(path.endsWith('/batches'))json={items:batch?[batch]:[]};else if(path.endsWith('/results')){state.queries++;json={items:[],statistics:batch?statistics:[],records:batch?records:[],comparison:{status:'comparable'}};}
