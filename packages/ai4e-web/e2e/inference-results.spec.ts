...
test('独立表格和坐标配置、聚合多选、样本身份及导出一致',async({page})=>{
 ...
 await expect(table.locator('tbody tr')).toHaveCount(4);await expect(table.locator('th')).toHaveCount(3);await expect(table).toContainText('训练集');await expect(table).toContainText('评价集');await expect(table).toContainText('same_sample');
