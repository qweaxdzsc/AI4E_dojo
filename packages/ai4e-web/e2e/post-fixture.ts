...
export async function postFixture(page:Page,{sameSampleAcrossSplits=false}={}){
 const state={creates:0,closes:0,appends:0,submitted:[] as any[],visible:[] as boolean[]};
 ...
 await page.route('**/api/v1/**',async route=>{const req=route.request(),u=new URL(req.url()),path=u.pathname;let json:any={};
  if(path==='/api/v1/projects')json=[{id:'p',name:'项目'}];else if(path.endsWith('/tasks/t'))json={id:'t',name:'任务',case_id:'shapenet_car_abupt',stage_summary:{infer:{status:'succeeded'}}};else if(path.endsWith('/tasks'))json=[{id:'t',name:'任务'}];else if(path.endsWith('/runs'))json=[];
   ...
   if(view==='catalog')json={items,files:[],batches:[{id:'batch1',name:'批次一'},{id:'batch2',name:'批次二'}],errors:[],total:0};
   else {
    const picked=files.filter(f=>(!batch||f.batch_id===batch)&&(!sample||f.sample===sample)&&(!q||f.tree_path.toLowerCase().includes(q.toLowerCase()))&&(!directory||f.tree_path===directory||f.tree_path.startsWith(directory+'/'))).map(f=>{
     if(q||directory||sample)return {...f,ref:undefined};
