...
for (const [name, status, body, expected] of [
  ...
  ['映射后的业务错误', 409, '{"detail":"processed_dataset_name_conflict: 已有登记 VTKHDF=关，当前配置 VTKHDF=开。确认后将覆盖原登记。","error":{"code":"processed_dataset_name_conflict","message":"该平台数据集名称已存在。未确认覆盖时不能提交：已有登记 VTKHDF=关，当前配置 VTKHDF=开。确认后将覆盖原登记。"}}', '该平台数据集名称已存在'],
