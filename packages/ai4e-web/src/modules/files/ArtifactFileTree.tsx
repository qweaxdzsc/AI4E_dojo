...
export function ArtifactFileTree({files,query='',selected=[],onSelection,onPreview,onVisualize,selectedFile,download,onExpand,loadingPaths=[],opened:openedProp,onOpened,selectable}:{files:ArtifactTreeFile[];query?:string;selected?:string[];onSelection?:(ids:string[])=>void;onPreview:(file:ArtifactTreeFile)=>void;onVisualize?:(file:ArtifactTreeFile)=>void;selectedFile?:string;download?:(file:ArtifactTreeFile)=>string;onExpand?:(path:string)=>void|Promise<void>;loadingPaths?:string[];opened?:string[];onOpened?:(paths:string[])=>void;selectable?:(file:ArtifactTreeFile)=>boolean}) {
 ...
 const render=(nodes:ArtifactNode[],depth=0):any=>nodes.flatMap(node=>{
  ...
  return [<tr key={node.key} className={(folder?'folderrow':'filerow')+(selectedFile===f?.id?' selected':'')}>
   ...
   <td className="artifact-name-cell"><button className="artifact-name" aria-label={node.name} style={{paddingLeft:depth*12}} title={node.path||node.name} aria-expanded={folder?expanded:undefined} onClick={()=>folder?void open(node):onPreview(f!)}>
   ...
   <td className="artifact-modified" title={f?.modified_at?new Date(f.modified_at).toLocaleString('zh-CN',{hour12:false}):undefined}>{folder||!f?.modified_at?'—':new Date(f.modified_at).toLocaleString('zh-CN',{hour12:false})}</td>
