...
import {openPhysicalWorkspace,closePhysicalWorkspace,physicalTargets,savedVisualizations,physicalSources,appendPhysicalSources,registerPhysicalSource} from './api';
import type {Source} from './model';
import './physical-workspace.css';

export interface PhysicalWorkspaceHandle {append:(sources:Source[])=>Promise<any>}
interface PhysFieldEmbedProps {sources:Source[];scope?:{project_id?:string;task_id?:string};mode?:string;active?:boolean;onError?:(e:Error)=>void}
const identity=(s:Source)=>JSON.stringify([s.project_id,s.asset_id,s.revision,s.member??null,s.block??null]);
/** iframe始终绑定同一会话；只有显式重开或离开任务时释放。后处理不展示宿主「打开已保存配置」，改由工作台文件菜单重开。 */
export const PhysFieldEmbed=forwardRef<PhysicalWorkspaceHandle, PhysFieldEmbedProps>(function PhysFieldEmbed({sources,scope,mode,active=true,onError},ref){
 ...
 useEffect(()=>{let live=true;if(mode!=='post'&&project&&task)savedVisualizations(project,task).then(v=>live&&setSaved(v.items.filter((i:any)=>i.kind==='phys_field'))).catch(e=>live&&fail(e));return()=>{live=false;};},[project,task,session,mode]);
 ...
 useEffect(()=>{const receive=async(event:MessageEvent)=>{
  ...
  try{
   let result;
   if(m.type==='ai4e-vis:session-open'){
    if(typeof m.visualization_id!=='string')throw new Error('缺少配置资产身份');
    result=await open(m.visualization_id);
   }else if(m.type==='ai4e-vis:source-list'){
    result=await physicalSources(target.project,target.task,m.root,m.path);
   }else{
    const sources=await Promise.all((m.sources||[]).map(async(source:any)=>{
     if(source?.ref?.asset_id)return source;
     const locator=source.locator||source;
     const ref=await registerPhysicalSource(target.project,{root:locator.root,path:locator.path,task_id:target.task});
     return {ref:{...ref,member:locator.member}};
    }));
    result=await appendPhysicalSources(target.project,target.task,target.session_id,sources);
   }
