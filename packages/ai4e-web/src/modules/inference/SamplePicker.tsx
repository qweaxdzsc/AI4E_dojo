...
export function SamplePicker({partitions,split,onSplit,selected,onChange,disabled,totalSelected=selected.length}: {
 partitions:Record<string,string[]>;split:string;onSplit:(s:string)=>void;selected:string[];onChange:(ids:string[])=>void;disabled:boolean;totalSelected?:number;
}) {
 ...
 return <section className="infer-card infer-picker" data-region="samples"><h3>2. 样本 <small>已选择 {totalSelected} / {Object.values(partitions).reduce((n,v)=>n+v.length,0)} 个</small></h3>
 ...
 <div className="infer-tabs" role="tablist" aria-label="推理分片">{Object.keys(partitions).map(s=><Button key={s} role="tab" aria-selected={split===s} className={s===split?"active":""} onClick={()=>onSplit(s)} disabled={disabled}>{splitLabel(s)} ({partitions[s].length.toLocaleString()})</Button>)}</div>
 ...
 <div className="infer-picker-list">{!partitions[split]?.length?<Empty image={Empty.PRESENTED_IMAGE_SIMPLE} description="选择检查点后读取样本"/>:visible.map(id=><div className="infer-choice" key={id}><Checkbox aria-label={"选择样本 "+id} title={id} disabled={disabled} checked={selection.has(id)} onChange={e=>toggle([id],e.target.checked)}>{id}</Checkbox></div>)}</div>
