...
/** 正式执行成功或设置页已保存才打勾；过期检查不把已保存清成未完成。 */
export function stageDisplay(task:any,index:number){
 ...
 const status=index===1&&fact?.status==='not_run'&&fact?.shared_input?.status==='available'?'available':fact?.status||(check?'unchecked':'not_run');
 const finished=status==='succeeded'||(check&&status==='stale');
 return {status,finished,active:['running','pending','queued','stopping','capturing'].includes(status),label:check&&finished?'已保存':labels[status]||'无法确认',reference:fact?.run_id||fact?.operation_id};
}
...
export function taskDisplay(task:any){
 ...
 if(stages.every(s=>s.finished||s.status==='available'))return {kind:'succeeded',label:'流程已完成'};
