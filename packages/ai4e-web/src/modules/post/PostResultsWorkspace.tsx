...
const TABS=[{id:'metrics',label:'指标'},{id:'files',label:'结果文件'},{id:'visualization',label:'三维物理场可视化'}];
/** 任务级后处理组合，Tab切换只改变可见性，绝不关闭主三维会话。 */
export function PostResultsWorkspace({project,task,batchId,runId,sample,split,tab:initialTab}:{project:string;task:string;batchId?:string;runId?:string;sample?:string;split?:string;resultIndex?:number;tab?:string}){
 const [tab,setTab]=useState(TABS.some(t=>t.id===initialTab)?initialTab!:(batchId||runId||sample?'files':'metrics')),[opened,setOpened]=useState(initialTab==='visualization'),[adding,setAdding]=useState(false),[error,setError]=useState(''),[notice,setNotice]=useState(''),[added,setAdded]=useState<string[]>([]);
 ...
 <section role="tabpanel" id="post-panel-metrics" aria-labelledby="post-tab-metrics" hidden={tab!=='metrics'}><PostMetricsPanel project={project} task={task} catalog={data.catalog} initialBatch={batchId} onExport={data.refresh} active={tab==='metrics'}/></section>
