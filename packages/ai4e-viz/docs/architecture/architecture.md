...
13 个模块表示职责划分，不表示全部需求已交付。自动化脚本持久化/执行与 MCP 服务尚未实现；几何选择/部件隐藏/结构树/剖切/测量仅部分状态预留；STEP、DXF、CSV/JSON 到 GLB 尚未接入；图片缺应用级缩放；报告删除、收藏和母版管理未开放。现行支持逐项见包内 PRD，以上不在本轮补充。

物理工作台仍由唯一 Workbench/场景持有状态；controller 委托 camera_controls、object_controls、session_controls、export_controls，分别处理相机、对象、播放及导出。委托对象不创建独立数据库、状态副本或服务。表所有权、存储键、公开路由和 Trame 协议保持。兼容根 Dojo 的稳定资产引用，不复制核心训练计算。
...
Trame 工作台由布局装配、控制器、工具栏、对象树、属性、视图与时间组件构成；React 仅组合保存/导入/导出弹窗。工作会话使用具名管线对象，显示实例通过输入对象与视图身份绑定；计算和着色字段分离。计算事务构建候选下游输出后提交，显示更新保留 actor/mapper 身份：显隐只更新目标及从属附件，透明度等样式只更新 property，着色才替换必要输入和映射设置，不运行上游过滤器。Trame State 使用其公开属性读写，不套用 dict.get；树状态与视口推送属于同一用例错误边界，失败回滚并报告。来源分组不执行批量显隐，父子依赖只用于计算与删除。首次切面草稿同步手柄，重复点击不推送；本地普通鼠标移动不发送平面 RPC，拖动以 40ms 合并并在释放时提交最终位置。画像按输入 MTime 复用，着色缓存按输入/字段 MTime、归属和分量最多保留四项；这些缓存不跨工作区。工作进程有界复用 renderer，避免 Trame 同步期间析构回调失效。
...
## 三维对象工作台的窗口与对象规则（2026-09-17）

保存的窗口类型仍可固定；实际出图按是否存在 Surface LIC 层在同一会话切换本地或远程挂件，不改已保存的窗口字段，也不整会话重建。导入由宿主列出任务产物、共享数据集和已挂数据根中的可视化网格，工作台只追加来源。管线对象增加线段提取；视口可选三维渲染或折线图，折线画在 Line Chart View。切面/剖切按皱折与三角化分支，流线种子附件可独立显隐。
