...
import {Modal, Form, InputNumber, Select, Alert, Checkbox} from 'antd';
/** 统一配置时序动画与显式图片、数据输出。 */
export function AnimationExportDialog({open, animation, formatHint='png', times=[], views=[], activeView, onCancel, onExport}) {
  ...
  const submit=()=>{const selected=times.filter((_,i)=>i>=start&&i<=end&&(i-start)%stride===0);if(reverse)selected.reverse();onExport({format,width,height,fps,transparent_background:transparent&&['png','png_sequence'].includes(format),...(view!=='all'?{view}:{}),...(['mp4','png_sequence'].includes(format)?{times:selected}:{})});};
  return <Modal open={open} title={animation?'生成时序动画':'导出图片 / 数据'} onCancel={onCancel} onOk={submit} okText="生成" okButtonProps={{disabled:(animation&&!times.length)||start>end}} destroyOnHidden maskClosable>
    ...
    <Form layout="vertical">
      ...
      {['png','png_sequence'].includes(format)&&<Form.Item><Checkbox checked={transparent} onChange={e=>setTransparent(e.target.checked)}>透明背景</Checkbox></Form.Item>}
    </Form>
  </Modal>;
}
