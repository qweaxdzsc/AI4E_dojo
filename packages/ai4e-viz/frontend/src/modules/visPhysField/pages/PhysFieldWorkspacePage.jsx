...
export default function PhysFieldWorkspacePage() {
 ...
 useEffect(()=>listenToFrame(frame,async message=>{
   try {
     ...
     else if(action==='import'){const sources=host?await hostCall('ai4e-vis:source-list'):(await apiArtifacts()).items;const listed=sources.items||sources;setItems((listed||[]).map(x=>({...x,id:x.id||x.asset_id||x.artifact_id,leaf:x.leaf??!x.directory})));setDialog('import');}
 ...
 return <section className="phys-workspace-shell">
   ...
   <ResultImportDialog open={dialog==='import'} items={items} onBrowse={host?async(root,path)=>{const result=await hostCall('ai4e-vis:source-list',{root,path});return result;} : undefined} onUpload={host?undefined:async file=>{try{const result=await apiUpload(file);const item=result.artifact||result;const value={...item,id:item.artifact_id||item.id,leaf:true};setItems(old=>[...old.filter(x=>x.id!==value.id),value]);return value;}catch(e){model.fail(e);return null;}}} onCancel={()=>setDialog('')} onImport={async item=>{try{if(host)await hostCall('ai4e-vis:source-append',{sources:[{locator:item,root:item.root,path:item.path,member:item.member}]});else await physApi.append(model.context,model.session.session_id,item.asset_id||item.id,item.member);setDialog('');}catch(e){model.fail(e);}}}/>
