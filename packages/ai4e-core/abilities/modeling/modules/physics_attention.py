...
"""物理切片与反切片；参考定义和本地修正记录见包内 Notice/physicsnemo/source.json。"""
...
def _project_input(
    ...
    """执行_project_input；张量布局、参数与返回值见下列参考说明。

    Project input through one or two learned layers and rearrange to multi-head format.
...
def _compute_slices_from_projections(
    ...
    """执行_compute_slices_from_projections；张量布局、参数与返回值见下列参考说明。
    ...
    if plus and proj_temperature is not None:
        ...
        clamped_temp = torch.clamp(temperature, min=0.5, max=5).to(slice_projections.dtype)
        slice_weights = nn.functional.softmax(slice_projections / clamped_temp, dim=-1)
    slice_weights = slice_weights.to(slice_projections.dtype)
    slice_norm = slice_weights.sum(1) + 0.01
...
class PhysicsAttentionBase(nn.Module, ABC):
    ...
    def _compute_slice_attention_te(self, slice_tokens: torch.Tensor) -> torch.Tensor:
        """执行_compute_slice_attention_te；张量布局、参数与返回值见下列参考说明。
    ...
    def _compute_slice_attention_sdpa(self, slice_tokens: torch.Tensor) -> torch.Tensor:
        """执行_compute_slice_attention_sdpa；张量布局、参数与返回值见下列参考说明。
    ...
    def _project_attention_outputs(
        ...
        """执行_project_attention_outputs；张量布局、参数与返回值见下列参考说明。
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        if not torch.compiler.is_compiling():
            ...
            if x.shape[-1] != self.dim:
                raise ValueError(f"Expected input feature dimension {self.dim}, got {x.shape[-1]}")
        projected = self.project_input_onto_slices(x)
        if self.plus:
            x_mid = projected
            fx_mid = x_mid
        else:
            x_mid, fx_mid = projected
