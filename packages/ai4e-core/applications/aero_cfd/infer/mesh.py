    ...
    for index in indices:
        ...
        with progress.unit(unit_samples) if progress else sample_context("mesh", unit_samples):
            ...
            if not surface_path.is_file() or not volume_path.is_file():
                raise FileNotFoundError(f"{VTK_NO_SOURCE_MESH}: {surface_path} / {volume_path}")
            ...
            if selected_domains is None or "surface" in selected_domains:
                with sample_context("mesh.surface.save", unit_samples):
                    ...
                    stamp_mesh_identity(surface_out, identity)
            ...
            if selected_domains is None or "volume" in selected_domains:
                with sample_context("mesh.volume.save", unit_samples):
                    ...
                    stamp_mesh_identity(volume_out, identity)
            ...
            written.append(
                {
                    ...
                    "kind": VTK_FULL_KIND,
                    "fields": {
                        "surface": ["pred_pressure", "gt_pressure"],
                        "volume": ["pred_velocity", "gt_velocity"],
                    },
...
def raw_mesh_paths(config, sample_id: str) -> tuple[Path, Path]:
    ...
    root = original_dataset_root(config)
    if root is None:
        raise FileNotFoundError(VTK_NO_SOURCE_ROOT)
