"""外流固定推理清单解释；成员引用由 Task 再检查路径及内容修订。"""
...
def read_results(payload):
    ...
    comparison = (
        inspect_artifacts({"operation": "compare", "reports": reports})
        if reports
    ...
    views = (
        inspect_artifacts({"operation": "result_views", "records": records})
        if records
