        ...
        records.append(
            {
                ...
                "production_wells": producers,
                "injection_wells": injectors,
            }
        )
    return records
...
def audit(root: Path):
    ...
    for field in expected_stats:
        for suffix in ["mean", "std"]:
            ...
            if (
                ...
                or (suffix == "std" and not (value > 0).all())
            ):
    ...
    for key, well_type in [
        ("Twh", "production_wells"),
        ("Hwh", "production_wells"),
        ("Ewh", "production_wells"),
        ("Pinj", "injection_wells"),
    ]:
    ...
    return {
        ...
        "distinct_training_inputs": len(training_ids),
        "distinct_demo_inputs": len(demo_ids),
        "overlapping_inputs": sorted(training_ids & demo_ids),
        "demo_fields_equal_author_prediction": duplicates,
        "demo_independent_truth": False,
        "training_fields": {
            "space": "author_normalized",
            "pressure_unit_after_decode": "MPa",
            "temperature_unit_after_decode": "degC",
        },
        ...
        "axes": ["sample", "x", "y", "z", "year"],
        "years": list(range(21)),
        "evaluation_scope": "24-case rotating validation is not an independent test set",
    }
