...
from local_components import ResearchSelection, epoch_records
...
def train(cfg, prepared=None):
    ...
    validation_record, validation_arrays = read_field_inputs(prepared, "validation")
    if validation_record["metadata"]["statistics"] != record["metadata"]["statistics"]:
        raise ValueError("训练与验证必须复用训练统计")
    contract["research"] = {
        "validation_fields": validation_record["fields"],
        "evaluate_every": cfg["train"]["evaluate_every"],
        "ema_decay": cfg["train"]["ema_decay"],
        "selection": "relative-objective-min-first-v1",
    }
    validation_batch = partial(
        named_array_batch,
        validation_arrays,
        device=device,
        names=(*binding.INPUT_NAMES, "target", "physical_target"),
    )
    ema = MovingAverage(model, decay=cfg["train"]["ema_decay"])
    policy = ResearchSelection(model, contract=contract)
        ...
        result = train_model(
            ...
            state_bindings={
                "selection": {
                    ...
                    "validate": policy.validate_state_dict,
                    "load": policy.load_state_dict,
                }
            },
        )
    result["resume_checkpoint"] = result["checkpoint"]
    if policy.selected is None:
        raise RuntimeError("训练未形成验证候选")
    result["checkpoint"] = str(session.checkpoint("best", policy.selected, namespace="train"))
    result["selection"] = policy.selector.state_dict()
