...
from local_components import validation_samples
...
def rawprep(cfg):
    ...
    samples = validation_samples(source.samples(), cfg["train"]["validation_count"])

    def read_sample(sample):
        return source.read({**sample, "split": sample.get("source_split", sample["split"])})
