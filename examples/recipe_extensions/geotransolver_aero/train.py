...
from variants import l1_objective
...
def train(cfg, prepared=None):
    ...
    job = fitting.configure_objectives(
        job, settings=cfg.model.get("supervision"), operation=l1_objective
    )
    job = fitting.configure_optimization(
        job,
        settings=cfg.train,
        optimizer_factory=components.model.optimizer_factory,
        scheduler_factory=components.model.scheduler_factory,
    )
