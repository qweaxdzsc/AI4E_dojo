...
from ai4e_contrib.application.operator_learning.preparation import original_samples, read_prepared
...
def infer(cfg, prepared=None, trained=None):
    ...
    if (
        ...
        or state["contract"]["components"] != cfg["components"]
        or state["contract"]["metadata"]["fields"] != record["metadata"]["fields"]
        or state["contract"]["metadata"]["units"] != record["metadata"]["units"]
