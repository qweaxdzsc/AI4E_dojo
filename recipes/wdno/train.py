...
def train(cfg, prepared=None):
    ...
    with cancellation() as stopped:
        result = fit(
            ...
            optimizer_factory=component(cfg["components"].get("optimizer")),
            scheduler_factory=component(cfg["components"].get("scheduler")),
            update_step=component(cfg["components"].get("update")),
            session=session,
            cancelled=stopped,
        )
