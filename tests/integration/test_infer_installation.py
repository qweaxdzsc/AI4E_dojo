...
def test_installed_task_and_server_inference(tmp_path):
    ...
    installed, installed_env = install_wheels(tmp_path, ("spec", "core", "contrib", "task", "server"))
...
import ai4e_task as task, ai4e_server, ai4e_spec, ai4e_core, ai4e_contrib
from ai4e_server.bootstrap.app import create_app
from ai4e_server.bootstrap.settings import Settings
from fastapi.testclient import TestClient
for module in (task, ai4e_server, ai4e_spec, ai4e_core, ai4e_contrib):
...
try:
    task.list_inference_checkpoints(project, item['id'])
except ValueError as exc:
    assert 'operation_unavailable' in str(exc), exc
else:
    raise AssertionError('free script must not acquire an implicit inference provider')
