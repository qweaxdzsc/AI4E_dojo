...
def test_migrate_formal_only_preserves_original_and_is_idempotent(tmp_path):
    ...
    assert task.migrate_shared_datasets(project, dry_run=False)[0]["status"] == "already_migrated"
...
def test_explicit_registered_names_preserve_exact_runs(tmp_path):
    ...
    assert task.migrate_shared_datasets(project)[0]["run_id"] == new["id"]
    assert task.migrate_shared_datasets(project, sources={}) == []
    with pytest.raises(ValueError, match="source_not_eligible"):
        task.migrate_shared_datasets(
            project, sources={"old": old["id"], "trial": trial["id"]}, dry_run=False
        )
    assert not (project / "shared/datasets").exists()
    selected = {"registered_old": old["id"], "registered_new": new["id"]}
    result = task.migrate_shared_datasets(project, sources=selected, dry_run=False)
    ...
    assert all(
        ...
        for r in task.migrate_shared_datasets(project, sources=selected, dry_run=False)
    )


def test_migration_failure_conflict_and_corrupt_repeat(tmp_path, monkeypatch):
    """复制失败撤下登记，同名须覆盖，已迁移副本损坏不能冒充幂等成功。"""
    from ai4e_task.projects import dataset_migration
    ...
    before = manifest.read_bytes()
    copy = dataset_migration.copy_physical_dataset

    def interrupted(*args):
        copy(*args)
        raise OSError("interrupted copy")

    monkeypatch.setattr(dataset_migration, "copy_physical_dataset", interrupted)
    with pytest.raises(OSError, match="interrupted copy"):
        task.migrate_shared_datasets(project, dry_run=False)
    assert task.get_shared_dataset(project, "sample_data")["status"] == "unavailable"
    assert manifest.read_bytes() == before
    monkeypatch.setattr(dataset_migration, "copy_physical_dataset", copy)
    with pytest.raises(FileExistsError, match="shared_dataset_exists"):
        task.migrate_shared_datasets(project, dry_run=False)
    task.migrate_shared_datasets(project, dry_run=False, overwrite=True)
